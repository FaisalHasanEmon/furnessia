...
const FeaturedProducts = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch("/api/featuredProducts.json");
                if (!response.ok) {
    ...
    return (
        ...
        <section className="max-w-480 mx-auto px-4 sm:px-6 lg:px-20 py-8 md:py-12 lg:py-24 space-y-4 bg-[#FAF8F3]">
            <CategoryAndViewAll
